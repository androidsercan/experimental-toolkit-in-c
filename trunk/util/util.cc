// $Id: util.cc 4340 2011-10-15 05:13:27Z haowu $
...
string Util::Format(double x, int n)
{
  char buff[50];
  sprintf(buff, "%.*f", n, x);
  return string(buff);
...
void Util::Split(const string& s, const string& d, vector<string>* p_result)
{
  ...
  string t = s;
  unsigned int cutAt;
  while ((cutAt = t.find_first_of(d)) != string::npos) {
    if (cutAt > 0) {
...
bool Util::IsSubSeq(const vector<int>& seq1, const vector<int>& seq2)
{
  ...
  for (int i = 0; i < static_cast<int>(seq1.size()); i++) {
    int v = seq1[i];
    int q = 0;
    Flag flag = ProbeSearch(v, seq2, p, seq2.size() - 1, &q);
    p = q;
    if (flag != SUCCESS) {
      return false;
    }
...
void Util::Isect_P(const vector<const vector<int>*>& zp_seq, vector<int>* p_result)
{
  ...
  for (int pos = 0; pos < static_cast<int>(ss.size()); pos++) {
    ...
    for (int i = 1; i < N; i++) {
      int p = 0;
      Flag flag = ProbeSearch(v, *zp_seq[i], z_pos[i], zp_seq[i]->size() - 1, &p);
      z_pos[i] = p;
      if (flag != SUCCESS) {
        is_result = false;
        break;
...
void Util::Normalize(int n, vector<double>* p_vec)
{
  double sum = 0;
  for (int i = 0; i < static_cast<int>(p_vec->size()); i++) {
    sum += pow((*p_vec)[i], (double)n);
  }
  for (int i = 0; i < static_cast<int>(p_vec->size()); i++) {
    (*p_vec)[i] /= sum;
  }
...
unsigned int Util::Rand()
{
  return Rand(sizeof(unsigned int) * 8);
}
...
unsigned int Util::Rand(unsigned int lb, unsigned int ub)
{
  if (lb == 0 && ub + 1 == 0) {
    return Rand(sizeof(unsigned int) * 8);
  }
                            ...
                            vector<int>* p_C)
{
  ...
  while (s != v) {
    ...
    for (int i = 0; i < static_cast<int>(D.size()); i++) {
      ...
      //double score = log((double)delta.size()) / (double)L[D[i]];
      //double score = (double)delta.size() / (double)L[D[i]];
      if (max <= score) {
        max = score;
        imax = i;
      }
...
Flag Util::BinarySearch(int key, const vector<int>& seq, int lb, int ub, int* p_pos)
{
  if (seq.size() == 0) {
    return FAILURE;
  }
...
Flag Util::ProbeSearch(int key, const vector<int>& seq, int lb, int ub, int* p_pos)
{
  if (seq.size() == 0) {
    return FAILURE;
  }
...
unsigned int Util::Rand(unsigned int n_bit)
{
  unsigned int x = 0;
