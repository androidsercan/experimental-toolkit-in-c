// $Id: heap.h 3581 2011-06-10 14:36:56Z haowu $
...
protected:
  //
  void Adjust(int pos_hole, T val_hole, int cat_hole);
  void Update(int pos, T val, int cat);
  //
  Comp  comp_;
  //
...
void Heap<T, Comp>::Push(const T& val, int cat)
{
  ...
  while (pos_hole > 0 && comp_(V_[pos_parent], val_hole)) {
    Update(pos_hole, V_[pos_parent], C_[pos_parent]);
    pos_hole = pos_parent;
    pos_parent = (pos_hole - 1) / 2;
  }
  Update(pos_hole, val_hole, cat_hole);
}
...
template<class T, class Comp>
void Heap<T, Comp>::Adjust(int pos_hole, T val_hole, int cat_hole)
{
  ...
  while (pos_child <= m_ - 1) {
    ...
    if (comp_(val_hole, V_[pos_child])) {
      Update(pos_hole, V_[pos_child], C_[pos_child]);
      pos_hole = pos_child;
...
template<class T, class Comp>
void Heap<T, Comp>::Update(int pos, T val, int cat)
{
  V_[pos] = val;
  C_[pos] = cat;
  P_[cat] = pos;
